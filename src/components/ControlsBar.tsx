...
import { Gauge, Cpu, RefreshCw, Languages } from 'lucide-react';
...
const selectClassName =
  'w-full bg-white/70 dark:bg-zinc-950/70 backdrop-blur-xs text-zinc-800 dark:text-zinc-200 border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl px-2.5 py-1.5 text-xs font-mono focus:outline-none focus:border-indigo-500 cursor-pointer disabled:opacity-50 transition-colors truncate';
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 p-3 rounded-2xl bg-white/50 dark:bg-zinc-900/40 backdrop-blur-md border border-white/70 dark:border-white/10 shadow-xs transition-colors">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
          <label htmlFor="model-picker" className="flex items-center gap-1.5">
      ...
      <div className="space-y-1.5 sm:col-span-2">
        ...
        >
          ...
          <span className="font-medium text-zinc-700 dark:text-zinc-300">{t.outputLanguageLabel}</span>
        </label>
        <select
          id="output-language"
